...
export const setElementAttribute = (elementId, attributeName, value) => {
    const element = getElement(elementId);
    if (element) element.setAttribute(attributeName, value || CONFIG.ERROR_FALLBACK_TEXT);
};
...
export const setTemplateElementText = (elementTemplateClone, targetId, textContent) => {
    const element = elementTemplateClone.querySelector(targetId);
    if (!element) throwElementNullError(targetId); 
    if (element) element.textContent = textContent || CONFIG.ERROR_FALLBACK_TEXT;
};

export const setTemplateElementAttribute = (elementTemplateClone, targetId, attributeName, value) => {
    const element = elementTemplateClone.querySelector(targetId);
    if (!element) throwElementNullError(targetId);
